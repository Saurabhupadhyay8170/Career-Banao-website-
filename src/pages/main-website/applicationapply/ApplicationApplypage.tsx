...
function ApplicationApplyPage() {
  ...
  if (location.pathname.includes("/admissions") && admlist?.findIndex((item) => item.module_name.includes(menuConfig.admEng)) != -1) {
    api = `/getApplicationDetailsEng?college_name=${collegeName}`;
    redirectLocation = "/admissions"
  ...
  else if (location.pathname.includes("/counselling") && counslist?.findIndex((item) => item.module_name.includes(menuConfig.counsEng)) != -1) {
    api = `/getCounslingDetails?college_name=${collegeName}`;
    redirectLocation = "/counselling"
  }
